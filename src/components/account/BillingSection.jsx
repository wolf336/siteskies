    ...
    <div className="space-y-4">
      ...
      {subscription?.status === 'past_due' && (
        <div className="rounded-lg bg-destructive/10 border border-destructive/30 text-destructive px-4 py-3 text-sm flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            >
              {openingPortal ? <Loader2 className="h-4 w-4 animate-spin" /> : t('billing.updatePayment')}
            </Button>
          </div>
        </div>
      )}
      ...
      {showSuccess && !pollTimedOut && tier !== 'free' && (
        <div className="rounded-lg bg-success/10 border border-success/30 text-success px-4 py-3 text-sm font-medium">
          {t('billing.subscriptionActivated', { plan: TIER_CONFIG[tier]?.name })}
        </div>
      )}
        ...
        <div className="space-y-4">
          {subscription?.effective_source === 'team' && subscription?.inherited_from && (
            <div className="rounded-lg bg-primary/5 border border-primary/20 px-4 py-3 text-sm">
              <strong className="text-foreground">{t('billing.teamPlanNote', { name: subscription.inherited_from.owner_name })}</strong>
              <span className="text-muted-foreground">{t('billing.teamPlanNoteDesc')}</span>
            </div>
          )}
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-foreground">{t('billing.currentPlan')}</h3>
            ...
            <CardContent className="space-y-3">
              {subscription?.current_period_end && (
                <p className="text-xs text-muted-foreground">
                  {subscription.cancel_at_period_end
                    ? t('billing.cancelsOn', { date: format(new Date(subscription.current_period_end), 'MMM d, yyyy') })
              ...
              {tier !== 'free' && subscription?.effective_source !== 'team' && (
                <Button
                  ...
                  disabled={openingPortal}
                >
                  {openingPortal ? <Loader2 className="h-4 w-4 animate-spin" /> : t('billing.manageSubscription')}
                </Button>
              )}
            </CardContent>
          </Card>
          ...
          {tier === 'free' && <>
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-foreground">{t('billing.availablePlans')}</h3>
            <Tabs value={billingInterval} onValueChange={setBillingInterval}>
              <TabsList>
                <TabsTrigger value="monthly">{t('billing.monthly')}</TabsTrigger>
                <TabsTrigger value="yearly">
                  {t('billing.yearly')} <span className="ml-1 text-[10px] text-success font-semibold">{t('billing.savePercent')}</span>
                </TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
